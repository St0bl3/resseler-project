...
export async function deliverKeysByEmail({ email, orderId, items }: DeliveryInput) {
  ...
  const generatedKeys = items.flatMap((item) =>
    Array.from({ length: item.quantity }, (_, index) => ({
      product: item.name,
      key: createDemoKey(orderId, item.name, index)
    }))
  );
...
function renderKeyEmail(orderId: string, keys: Array<{ product: string; key: string }>) {
  const rows = keys
    .map(
      (item) => `
        <tr>
          <td style="padding:12px;border-bottom:1px solid #16304d;color:#c8e9ff;">${item.product}</td>
          <td style="padding:12px;border-bottom:1px solid #16304d;color:#67e8f9;font-weight:700;">${item.key}</td>
        </tr>
      `
    )
    .join("");
  ...
  return `
    <div style="background:#030814;color:#f8fafc;font-family:Arial,sans-serif;padding:28px;">
      <h1 style="margin:0 0 8px;">ResselerHub</h1>
      <p style="color:#a8c7dd;">Спасибо за покупку. Заказ: ${orderId}</p>
