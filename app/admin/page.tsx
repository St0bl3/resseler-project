...
import { KeyRound, RotateCcw, Save, ShieldCheck, Trash2 } from "lucide-react";
import { FormEvent, useMemo, useState } from "react";
import { useShopData } from "@/lib/shop-storage";
    ...
    <main className="min-h-screen bg-[#02030a] text-slate-50">
      ...
      <section className="relative z-10 mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-4 border-b border-white/10 pb-6 md:flex-row md:items-end md:justify-between">
          <div>
            <div className="inline-flex items-center gap-2 rounded-lg border border-emerald-300/20 bg-emerald-300/10 px-3 py-2 text-sm font-bold text-emerald-100">
              <ShieldCheck className="size-4" />
              Закрытая зона
            </div>
            <h1 className="mt-4 text-4xl font-black text-white sm:text-5xl">Панель ResselerHub</h1>
            <p className="mt-3 max-w-2xl text-slate-300">
              Управляй товаром, видимостью и ключами. Обычные пользователи не видят ссылку на эту страницу, а вход закрыт серверной авторизацией.
            </p>
          </div>
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
              ...
              {products.map((product) => (
                ...
                >
                  <span>
                    ...
                    <small>
                      {product.active ? "Активен" : "Скрыт"} · {product.stock} ключей
                    </small>
                  </span>
                  <span className="text-cyan-100">${product.variants[0]?.price ?? 0}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          {selectedProduct ? (
            <div className="admin-panel">
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      updateProduct(selectedProduct.id, {
                        variants: selectedProduct.variants.map((variant, index) =>
                          index === 0 ? { ...variant, price: Number(event.target.value) } : variant
                        )
                      })
                    }
                  />
              ...
              <form className="mt-6 rounded-lg border border-white/10 bg-[#040710] p-4" onSubmit={handleAddKeys}>
